...
namespace DRGN.Projectiles
{
	...
	public class ElectroStaffBolt : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor) {
			...
			if (projectile.ai[0] == -2 && Segments.Count > 0 )
			{
...
//				spriteBatch.End();
		...
		protected static List<Line> CreateBolt(Vector2 source, Vector2 dest, int  thickness)
		{
			...
			for (int i = 1; i < positions.Count; i++)
			{
				...
				// fscale > 0 and < 1 
				float fscale = length * (pos - positions[i - 1]) * BoltJagged;
				...
				//	Main.NewText("i " + i + " pos " + pos + " -1 " + positions[i - 1] + " diff " + (BoltJagged * length * (pos - positions [i-1])) + " jag " + BoltJagged, 60, 60, 60 );
				//}
