...
{
    public class ShadeBlast : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            ...
            else
            {
                for (int i = 0; i < 40; i++)
                {
                    int dustid = Dust.NewDust(projectile.position, projectile.width, projectile.height, mod.DustType("ShadeDust"), Main.rand.NextFloat(-10, 10), Main.rand.NextFloat(-10, 10));
                    Main.dust[dustid].noGravity = true;
                }
            }
        }
