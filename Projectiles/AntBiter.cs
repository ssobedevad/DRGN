...
namespace DRGN.Projectiles
{
    ...
    public class AntBiter : ModProjectile
    {
        ...
        public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 5)
        {
            ...
            for (float i = transDist; i <= Distance; i += step)
            {
                ...
                spriteBatch.Draw(texture, origin - Main.screenPosition,
                    new Rectangle(0, 32, 30, 32), i < transDist ? Color.Transparent : c, r,
                    new Vector2(30 * .5f, 32 * .5f), scale, 0, 0);
            }

            // Draws the laser 'tail'
            spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
                new Rectangle(0, 0, 32, 30), Color.White, r, new Vector2(32 * .5f, 30 * .5f), scale, 0, 0);

            // Draws the laser 'head'
            spriteBatch.Draw(texture, start + (Distance + step) * unit - Main.screenPosition,
                new Rectangle(0, 64, 32, 64), Color.White, r, new Vector2(32 * .5f, 64 * .5f), scale, 0, 0);
        }
