	...
	public abstract class VoidSnakeAI : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (halfHealthSpawn) { speed = (DRGNModWorld.MentalMode ? 22 : Main.expertMode ? 20 : 18); turnSpeed2 = (DRGNModWorld.MentalMode ? 0.75f : Main.expertMode ? 0.5f : 0.25f); }
				if (tenthHealthSpawn) { speed = (DRGNModWorld.MentalMode ? 32 : Main.expertMode ? 28 : 25); turnSpeed2 = (DRGNModWorld.MentalMode ? 1f : Main.expertMode ? 0.75f : 0.5f); }
			...
			else if (npc.localAI[2] == 1)
			{
				if(playerCenterToCircle == new Vector2(-1, -1)) 
				{
					...
					if (Main.netMode != 1 && head)
					{
						for (int i = 0; i < 6; i++)
						{ NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<VoidEye>(), 0, playerCenterToCircle.X, playerCenterToCircle.Y, i * 1.05f); }
					}
				...
				if (head)
				{
					...
					if (npc.localAI[3] >( 1.8f * (laserPattern+1))) { Projectile.NewProjectile(playerCenterToCircle + laserSequence1[laserPattern].XY(), laserSequence1[laserPattern].ZW(), ProjectileType<VoidBeamWarning>(), npc.damage/10, 0f); laserPattern++; if (laserPattern % 16 == 0) { npc.localAI[3] = 0; laserPattern = 0; }
