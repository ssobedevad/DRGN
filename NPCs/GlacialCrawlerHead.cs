...
namespace DRGN.NPCs
{
    public class GlacialCrawlerHead : ModNPC
    {
        ...
        {
            if (DRGNModWorld.downedIceFish && healthUpdate == false) { if (!Main.expertMode) { npc.damage = 150; npc.lifeMax = 15000; npc.life = 15000; } else { npc.damage =300; npc.lifeMax = 30000; npc.life = 30000; }healthUpdate = true; }
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("GlacialCrawlerBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("GlacialCrawlerTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
            ...
            if (NPC.AnyNPCs(mod.NPCType("GlacialCrawlerBody")) == false) { npc.active = false; }
